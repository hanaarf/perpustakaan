...
// Impor PhpSpreadsheet
require '../../vendor/autoload.php'; // Gantilah '/path/to/vendor' dengan jalur ke direktori vendor PhpSpreadsheet
...
// Inisialisasi data yang akan di-export ke Excel
$dataToExport = array();
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['export'])) {
        ...
        // Set judul header berdasarkan pilihan pengguna
        $header = array();
        ...
        // Menulis judul header ke Excel
        $col = 1;
        foreach ($header as $item) {
            $sheet->setCellValueByColumnAndRow($col, 1, $item);
            $col++;
        }
        ...
        foreach ($html->find('oai_dc:dc') as $t) {
            ...
            if ($date >= $startYear && $date <= $endYear) {
                ...
                foreach ($selectedColumns as $column) {
                    if ($column == 'title') {
                        $sheet->setCellValueByColumnAndRow($col, $row, addslashes($t->find('dc:title', 0)->innertext));
                    } elseif ($column == 'creator') {
                        ...
                        $sheet->setCellValueByColumnAndRow($col, $row, implode(', ', $creators));
                    } elseif ($column == 'publisher') {
                        $sheet->setCellValueByColumnAndRow($col, $row, addslashes($t->find('dc:publisher', 0)->innertext));
                    } elseif ($column == 'date') {
                        $sheet->setCellValueByColumnAndRow($col, $row, addslashes($t->find('dc:date', 0)->innertext));
                    } elseif ($column == 'language') {
                        $sheet->setCellValueByColumnAndRow($col, $row, addslashes($t->find('dc:language', 0)->innertext));
                    } elseif ($column == 'type') {
                        $sheet->setCellValueByColumnAndRow($col, $row, addslashes($t->find('dc:type', 0)->innertext));
                    } elseif ($column == 'format') {
                        $sheet->setCellValueByColumnAndRow($col, $row, addslashes($t->find('dc:format', 0)->innertext));
                    } elseif ($column == 'coverage') {
                        $sheet->setCellValueByColumnAndRow($col, $row, addslashes($t->find('dc:coverage', 0)->innertext));
                    }
                    $col++;
                }
                $row++;
        ...
        $exportedFileName = 'exported_data.xlsx';
        $writer->save($exportedFileName);
        ...
        // Hapus file sementara yang dihasilkan
                    ...
                    <div class="col-xl-12 col-xxl-12">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">Form Export Data</h4>
                            </div>
                            <div class="card-body">
                                ...
                                <form method="post">
                                    ...
                                    <input type="submit" value="Export Data ke Excel">
                                </form>
                            </div>
                        </div>
                    </div>
